...
export async function allocateSaleFIFOCandy(params: {
  ...
  // 1) Determinar si la llamada viene en PAQUETES o en UNIDADES
  ...
  // Traer lotes del producto para:
  //  - ordenar FIFO
  //  - obtener unitsPerPackage para convertir paquetes -> unidades
  const qB = query(
    collection(db, "inventory_candies"),
    where("productId", "==", productId)
  );
  ...
  if (packsRaw != null && !Number.isNaN(Number(packsRaw))) {
    ...
    // Compatibilidad: usan quantity en UNIDADES como antes
  ...
  await runTransaction(db, async (tx) => {
    ...
    for (let i = 0; i < refs.length && need > 0; i++) {
      ...
      // 🔵 calculamos paquetes remanentes
      ...
      tx.update(ref, {
        ...
        // mantenemos packages alineado con remainingPackages
    ...
    if (need > 0) {
      // Convertimos a paquetes para el mensaje de error
...
/**
 * Intenta extraer allocations que vienen del PEDIDO DEL VENDEDOR
 * (inventario_candies_sellers) desde sale.allocationsByItem.
 *
 * Estructura esperada (como la que guarda SalesCandiesPOS):
 * allocationsByItem: {
 *   [productId]: {
 *     productId: string;
 *     vendorId: string;
...
/**
 * Restaura inventario para una venta de DULCES y elimina la venta.
 *
 * 👉 NUEVO COMPORTAMIENTO:
 * - Si la venta tiene allocationsByItem con `inventorySellerId`:
 *    ✅ Restaura SOLO el inventario del VENDEDOR
 *       (colección `inventory_candies_sellers`, campos remainingUnits/remainingPackages).
 *
 * - Si NO tiene ese formato (ventas viejas):
 *    ✅ Usa la lógica legacy y restaura sobre `inventory_candies`
 *       usando `allocations` / `items[i].allocations`.
 *
 * - NO toca CxC (ar_movements).
 */
export async function restoreSaleAndDeleteCandy(saleId: string) {
  const baseId = saleId.split("#")[0]; // por si viene con sufijo #idx
  ...
  /* ---------------------------------------------------------------------- */
  /* 1) PRIMERO INTENTAMOS RESTAURAR PEDIDO DEL VENDEDOR (nuevo POS)        */
  /* ---------------------------------------------------------------------- */
  ...
  if (sellerAlloc && sellerAlloc.allocations.length > 0) {
    ...
    await runTransaction(db, async (tx) => {
      // Agrupar unidades por inventorySellerId
  ...
  /* ---------------------------------------------------------------------- */
  /* 2) LEGACY: RESTAURAR INVENTARIO GENERAL (inventory_candies)            */
  /* ---------------------------------------------------------------------- */
    ...
    await runTransaction(db, async (tx) => {
      ...
      for (let i = 0; i < refsToUpdate.length; i++) {
        ...
        tx.update(refsToUpdate[i].ref, {
          remaining: newRem,
          remainingPackages: newRemainingPackages,
          // sincronizamos packages cuando devolvemos inventario
          packages: newRemainingPackages,
        });
      }
  ...
  await runTransaction(db, async (tx) => {
    ...
    for (const it of items) {
      ...
      // Fallback FIFO: primero donde hubo consumo (baseUnits - remaining)
