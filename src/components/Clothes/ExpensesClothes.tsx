// src/components/Clothes/ExpensesClothes.tsx
...
export default function ExpensesClothes() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const ref = await addDoc(collection(db, "expenses_clothes"), {
        date: dateStr,
        category,
  ...
  const saveEdit = async () => {
    ...
    try {
      await updateDoc(doc(db, "expenses_clothes", editingId), {
        date: eDate,
        category: eCategory,
  ...
  const handleDelete = async (r: ExpenseRow) => {
    ...
    if (!ok) return;
    await deleteDoc(doc(db, "expenses_clothes", r.id));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="text-2xl font-bold mb-3">Gastos (Ropa)</h2>
